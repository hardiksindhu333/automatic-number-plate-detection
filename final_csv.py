...
for row in interpolated_data:
    car_id = row['car_id']
    if car_id not in highest_score_rows or float(row['license_number_score']) > float(highest_score_rows[car_id]['license_number_score']):
        highest_score_rows[car_id] = row

# Write all rows with the highest score for each car_id to a new CSV file
header = ['frame_nmr', 'car_id', 'car_bbox', 'license_plate_bbox', 'license_plate_bbox_score', 'license_number', 'license_number_score']
with open('interpolate_2.csv', 'w', newline='') as file:
    writer = csv.DictWriter(file, fieldnames=header)
    writer.writeheader()
    writer.writerows(highest_score_rows.values())
